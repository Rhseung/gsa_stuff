...
// 후위표기식 계산
double calculate(char postfixed[]);
...
int main() {
    ...
    printc(GRAY, "\n");
    center("------------------------------------------------\n");
    center("| formula | help | settings | utilities | quit |\n");
    center("------------------------------------------------\n");
    ...
    while (true) {
        ...
        // 명령어가 수식 계산이라면
            ...
            // 후위표기식으로 변환
            bool result = postfix(formula, postfixed);
            if (!result) continue;

            printc(GRAY, "\n");
            // 계산
            double caled = calculate(postfixed);
            printc(WHITE, "\n");
        }
    }
...
double calculate(char postfixed[]) {
    ...
    return atof(ctx[0].str);
}

void analyse(double n) {
    /*
    * 약수
    * 소인수분해
    * 이진수
